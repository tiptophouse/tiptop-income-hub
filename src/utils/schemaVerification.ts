  ...
  const verifySchema = async () => {
    ...
    try {
      ...
      try {
        ...
        const { data: tableData, error: rpcError } = await (supabase.rpc as any)('get_tables');
        
        if (!rpcError && tableData) {
          const existingTables = Array.isArray(tableData) 
            ? tableData.map((item: any) => item.table_name) 
            : [];
