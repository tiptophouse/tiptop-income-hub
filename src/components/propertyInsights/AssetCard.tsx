    ...
    <div className={`relative overflow-hidden rounded-2xl ${glassStyle} group hover:scale-[1.02] transition-all duration-300 border-l-4 ${color} cursor-pointer h-full`}>
      ...
      <CardHeader className="pb-1 pt-3 pr-2 bg-transparent flex flex-col">
        <div className="flex items-center gap-2 mb-1">
          ...
          <CardTitle className={`${isMobile ? 'text-sm' : 'text-sm md:text-lg'} font-bold text-[#6E59A5] truncate max-w-[90%]`}>{main}</CardTitle>
        </div>
        <CardDescription className={`${isMobile ? 'text-xs' : 'text-xs md:text-sm'} font-medium text-gray-700 line-clamp-2 min-h-[2.5rem]`}>{details}</CardDescription>
      </CardHeader>
      <CardContent className={`pt-0 ${isMobile ? 'pb-2' : 'pb-3 md:pb-4'}`}>
        <div className={`${isMobile ? 'text-lg' : 'text-xl md:text-2xl'} font-extrabold text-[#8B5CF6]`}>{value}</div>
      </CardContent>
    </div>
  );
};
