...
const HowItWorks = () => {
  ...
  return (
    <section id="how-it-works" className="py-10 md:py-16 lg:py-24 px-4 md:px-8 lg:px-12 max-w-7xl mx-auto">
      <motion.h2 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 lg:gap-12">
        {steps.map((step, index) => (
          <motion.div 
            key={index}
            className="bg-white rounded-xl p-6 md:p-8 shadow-md flex flex-col items-center text-center"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: index * 0.2 }}
          >
            <div className="mb-4 md:mb-6 bg-tiptop-light rounded-full p-3 md:p-4">
              {step.icon}
            </div>
            <h3 className="text-lg md:text-xl font-semibold mb-2 md:mb-3">{step.title}</h3>
            <p className="text-sm md:text-base text-tiptop-dark/70">{step.description}</p>
          </motion.div>
        ))}
      </div>
    </section>
