      ...
      <section className="pt-16 pb-24 px-4 md:px-8 lg:px-12 flex flex-col items-center text-center max-w-6xl mx-auto">
        ...
        delay: 0.4
          ...
          <div className="relative">
            <Input type="text" placeholder="Enter your property address..." className="pl-12 pr-16 sm:pr-28 py-6 w-full rounded-full text-base sm:text-lg shadow-sm" value={address} onChange={e => setAddress(e.target.value)} />
            <Search className="absolute left-5 top-1/2 transform -translate-y-1/2 h-5 w-5 text-muted-foreground" />
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-2">
              <Button type="button" variant="ghost" size="icon" className="p-1 h-9 w-9 rounded-full" onClick={handleLocationDetection} disabled={isLocating}>
                {isLocating ? <span className="flex items-center">
                    <svg className="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                  </span> : <MapPin className="h-4 w-4" />}
              </Button>
              <Button type="submit" className="bg-tiptop-accent hover:bg-tiptop-accent/90 text-white px-6 py-3 h-auto rounded-full text-sm font-medium">
                Analyze Now
              </Button>
            </div>
          </div>
